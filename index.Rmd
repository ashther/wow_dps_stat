...
p <- data %>% 
  ...
  geom_text(data = distinct(data, name, dps_median, spec), aes(name, dps_median, label = spec), nudge_y = 300, alpha = 0.6, size = 5) + 
  coord_flip() + 
  scale_fill_manual(values = pal_wow) + 
  theme_minimal() + 
  labs(x = NULL, y = sprintf('updated @ %s', as.character(file_info$mtime)))
